...
colnames(crime_data) <- c("y", "M", "So", "Ed", "Po1", "Po2", "LF", "M.F", "Pop", "NW", "U1", "U2", "Wealth", "Ineq", "Prob", "Time")
...
g <- ncol(X)  # Set g to the number of columns in X (number of predictors)
nu0 <- 2
s20 <- 1
S <- 1000
n <- dim(X)[1]
...
#Obtaining marginal posterior means for beta and OLS estimates

ols_estimates <- solve(t(X)%*%X)%*%t(X)%*%y
ols_estimates

marg_mean <- colMeans(beta)
marg_mean

marg_mean -ols_estimates
...
i.te<-sample(1:n,size=floor(n/2))
i.tr<-(1:n)[-i.te]

y.tr<-yf[i.tr] ; y.te<-yf[i.te]
X.tr<-Xf[i.tr,]; X.te<-Xf[i.te,]

#Calculating train ols estimate for beta

ols_train <- (solve(t(X.tr) %*% X.tr) %*% t(X.tr) %*% y.tr)
ols_train

#Calculation predicted test values

ols_y <- X.te%*%ols_train
ols_y

#Plotting test versus predicted 

plot(y.te,ols_y,xlab=expression(italic(y)[test]),ylab=expression(hat(italic(y))[OLS]))
abline(0,1)

#Calculating prediction error

pred_err1 <- (1/length(X.te))*sum((ols_y-y.te)^2)
pred_err1

#Calculating Bayes's estimator

bayes_b <- (g/(1+g))*ols_train
bayes_b

#Calculating y using bayes's estimator

y_bayes <- X.te%*%bayes_b
y_bayes

#Plotting against test data

plot(y.te,y_bayes,xlab=expression(italic(y)[test]),ylab = expression(hat(italic(y))[Bayes]))
abline(0,1)

pred_err2 <- (1/length(X.te))*sum((y_bayes-y.te)^2)
pred_err2

#Computing for many different seeds
...
for (i in 1:1000){
  ...
  ols_error[i] <- (1/length(X.te))*sum((ols_y-y.te)^2)
  
  bayes_b <- (g/(1+g))*ols_train
  y_bayes <- X.te%*%bayes_b
  
  bayes_error[i] <- (1/length(X.te))*sum((y_bayes-y.te)^2)
}

#Calaculating the mean of both the errors from bayesian and ols

mean_bayes_error <- mean(bayes_error)
mean_ols_error <- mean(ols_error)
